...
> Karrot-inspired local marketplace UI: warm orange accent, neighborhood trust patterns, simple listings, rounded controls, and Seed Design-informed spacing.

Source adaptation: Design System Inspiration of Karrot (당근).
...
Adapted from `oh-my-design-cli@1.2.0` / `web/references/karrot/DESIGN.md` (MIT). The referenced brand names, logos, and official design materials belong to their respective companies. This DESIGN.md is an aesthetic inspiration for AI-assisted UI generation, not official brand documentation.
