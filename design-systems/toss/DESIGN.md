...
> Toss-inspired fintech UI: calm white surfaces, precise blue actions, strong financial numerals, mobile-first flows, and restrained trustworthy motion.

Source adaptation: Design System Inspiration of Toss (토스).
...
Adapted from `oh-my-design-cli@1.2.0` / `web/references/toss/DESIGN.md` (MIT). The referenced brand names, logos, and official design materials belong to their respective companies. This DESIGN.md is an aesthetic inspiration for AI-assisted UI generation, not official brand documentation.
