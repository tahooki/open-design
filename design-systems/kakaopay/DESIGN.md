...
> KakaoPay-inspired friendly fintech UI: Kakao yellow as an ecosystem anchor, soft rounded geometry, accessible Korean-first typography, and low-stress payment flows.

Source adaptation: Design System Inspiration of KakaoPay.
...
Adapted from `oh-my-design-cli@1.2.0` / `web/references/kakaopay/DESIGN.md` (MIT). The referenced brand names, logos, and official design materials belong to their respective companies. This DESIGN.md is an aesthetic inspiration for AI-assisted UI generation, not official brand documentation.
