  ...
  for (const ev of events) {
    if (ev.kind === 'status') {
      if (ev.label === 'starting') {
        ...
        items.push({ key: `s-${statusIndex++}`, label: 'Opening Codex session' });
    ...
    if (ev.kind === 'debug') {
      if (ev.label === 'spawned') {
        ...
        items.push({
          key: `d-${debugIndex++}`,
          label: ev.detail?.includes('ERROR') ? 'Runtime error output' : 'Runtime warning',
          detail: ev.detail,
          ts: ev.ts,
          tone: ev.detail?.includes('ERROR') ? 'warn' : 'normal',
        });
      }
      continue;
    }
...
function ActivityLog({
  ...
  return (
    <div className="activity-log" data-streaming={streaming ? 'true' : 'false'}>
      <button
        type="button"
        ...
        aria-expanded={open}
      >
        <span className="activity-log-pulse" data-active={streaming ? 'true' : 'false'} />
      ...
      <div className="activity-log-lines">
        {visible.map((item) => (
          <div className="activity-line" data-tone={item.tone ?? 'normal'} key={item.key}>
...
function summarizeRuntime(events: AgentEvent[]): RuntimeSummary | null {
  ...
  return { pid, config, lastActivity, latestStatus, latestDebug, files, stderrCount };
}
